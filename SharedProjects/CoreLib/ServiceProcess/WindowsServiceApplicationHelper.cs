...
namespace Mohammad.ServiceProcess
{
    public static class WindowsServiceApplicationHelper
    {
        ...
        public static void Run(IEnumerable<ServiceBase> winServices, Action intractiveAction, ExceptionHandling exceptionHandling = null, bool autoStop = false)
        {
            if (!Debugger.IsAttached)
            {
                Catch(() => ServiceBase.Run(winServices.ToArray()), exceptionHandling);
            }
            else
            {
                var task = Async.Run(() => { Catch(intractiveAction, exceptionHandling); });
                if (autoStop)
