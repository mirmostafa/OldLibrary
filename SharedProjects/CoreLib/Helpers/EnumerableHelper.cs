...
namespace Mohammad.Helpers
{
    ...
    public static partial class EnumerableHelper
    {
        ...
        /// <param name="source"> The source. </param>
        /// <returns> </returns>
        public static object[] ToArray(params object[] source)
        {
            return source;
        }
        ...
        /// <returns> </returns>
        public static T[] ToArray<T>(params T[] ts)
        {
            return ts;
        }
        ...
        /// <exception cref="ArgumentNullException">dict</exception>
        public static void AddMany<T, TKey, TValue>(this IDictionary<TKey, TValue> dict, IEnumerable<T> newItems, Func<T, TKey> key,
            Func<T, TValue> value)
        {
            if (dict == null)
                throw new ArgumentNullException(nameof(dict));
        ...
        /// <param name="groupSize"> Size of the group. </param>
        /// <returns> </returns>
        public static IEnumerable<IEnumerable<T>> TakeGroups<T>(IEnumerable<T> items, int groupSize) => TakeGroups(items, groupSize, groupSize);
        ...
        /// <returns> </returns>
        public static IEnumerable<TResult> Zip<TResult, T1, T2>(IEnumerable<T1> source1, IEnumerable<T2> source2,
        ...
        public static bool Contains<TSource>(this IEnumerable<TSource> source, TSource item, Comparison<TSource> comparison) =>
            source.Any(currItem => comparison(currItem, item) == 0);

        public static bool Contains<TSource>(this IEnumerable<TSource> source, TSource item,
            Func<TSource, TSource, bool> comparer) => source.Any(currItem => comparer(currItem, item));
        ...
        public static IEnumerable<TDestination> ForEachFunc<TSource, TDestination>(this IEnumerable<TSource> source,
            Func<TSource, TDestination> actor)
        {
            if (source == null)
                throw new ArgumentNullException(nameof(source));
            if (actor == null)
        ...
        public static IEnumerable IfZero(IEnumerable source, IEnumerable defaultSource) => !source.Cast<object>().Any()
            ? defaultSource
            : source;
        ...
        public static IEnumerable<T> ToEnumerable<T>(T obj) { yield return obj; }
        ...
        public static IEnumerable<KeyValuePair<T, T>> ToPairs<T>(this IEnumerable<T> items)
        {
            return items.Select(item => new KeyValuePair<T, T>(item, item));
        }
        ...
        public static void ForEachTreeNode<T>(this IEnumerable<T> roots, Func<T, IEnumerable<T>> getChildren, Action<T> rootAction,
            Action<T, T> childAction)
            where T : class
        {
        ...
        public static void BuildTree<TSource, TItem>(this IEnumerable<TSource> rootElements, Func<TSource, TItem> getNewItem,
            Func<TSource, IEnumerable<TSource>> getChildren, Action<TItem> addToRoots, Action<TItem, TItem> addChild)
        {
            if (getChildren == null)
        ...
        public static IEnumerable<T> CastOrNull<T>(this IEnumerable source)
            where T : class => Cast<T>(source, item => item.As<T>());
        ...
        public static IEnumerable<T> Merge<T>(params IEnumerable<T>[] sources) { return sources.SelectMany(source => source); }

        public static void RemoveRange<TEntity>(this ICollection<TEntity> source, IEnumerable<TEntity> entities)
        {
            entities.ForEach(e => source.Remove(e));
        }

        public static void RemoveRange<TEntity>(this ICollection<TEntity> source, Predicate<TEntity> match)
        {
            RemoveRange(source, source.Where(entity => match(entity)));
        }

        public static void RemoveRange<TEntity>(this IList<TEntity> source, IEnumerable<TEntity> entities)
        {
            entities.ForEach(e => source.Remove(e));
        }

        public static void RemoveRange<TEntity>(this IList<TEntity> source, Predicate<TEntity> match)
        {
            RemoveRange(source, source.Where(entity => match(entity)));
        }
        ...
        public static void CleanNulls<TSource>(this IList<TSource> source)
            where TSource : class
        {
            source.RemoveRange(t => t == null);
        }

        public static IEnumerable<TSource> RemoveDefaults<TSource>(this IEnumerable<TSource> source,
            TSource defaultValue = default(TSource)) => defaultValue == null
            ? source.Where(item => item != null)
            : source.Where(item => item.Equals(defaultValue));

        public static bool Any(this IEnumerable source) { return Enumerable.Any(source.Cast<object>()); }
        ...
        {
            var list = source as IList<T> ?? source.ToList();
            return list.Count() - 1 == list.IndexOf(item) ? default(T) : list.ElementAt<T>(list.IndexOf(item) + 1);
        }
        ...
        public static IEnumerable<T> FlattenTreeNode<T>(this IEnumerable<T> e, Func<T, IEnumerable<T>> getChildren) => e.ToList()
        ...
        public static IEnumerable<T> FlattenTreeNode<T>(this IList<T> e, Func<T, IEnumerable<T>> getChildren) => e
            .SelectMany(c => getChildren(c).FlattenTreeNode(getChildren)).Concat(e);
        ...
        public static IEnumerable<TResult> Merge<TResult, T>(params IEnumerable<T>[] source)
            where T : TResult
        {
            return source.SelectMany(items => items.Cast<TResult>());
        }
        ...
        public static IEnumerable<TData> Except<TData>(this IEnumerable<TData> source, TData item, Predicate<TData> predicate)
        {
            if (predicate == null)
                throw new ArgumentNullException(nameof(predicate));
            return source.Where(data => !predicate(data));
        }

        public static IEnumerable<T> Lookup<T>(this IEnumerable<T> source, Func<T, IEnumerable<T>> getChildren,
        ...
        public static IEnumerable<T> Distinct<T>(this IEnumerable<T> source, Func<T, T, bool> comparer, Func<T, int> getHashCode = null)
            => source.Distinct(new LibEqualityComparer<T>(comparer, getHashCode));

        public static IEnumerable<T> Distinct<T>(this IEnumerable<T> source, Func<T, object> getProp) => source.GroupBy(getProp)
            .Select(x => x.First());

        public static bool HasDuplicates<TEnum, TPropType>(this IEnumerable<TEnum> source, Func<TEnum, TPropType> fieldSelector) => source
            .GroupBy(fieldSelector).Any(grp => grp.ElementAtOrDefault(1) != null);
        ...
        public static void Iterate<TSource>(this IEnumerable<TSource> source, int steps, Action<TSource> action)
            where TSource : class
        {
            source?.Iterate(steps).ForEach(action);
        }

        public static IEnumerable<T> Paginate<T>(this IEnumerable<T> query, (int PageCount, int? PageIndex) paging) => query.Paginate(
            paging.PageCount,
            paging.PageIndex);
