...
namespace Mohammad.Net
{
    public class Ftp : IExceptionHandlerContainer, ILoggerContainer
    {
        ...
        public static Ftp CreateInstance(string hostIp, string userName, string password, ExceptionHandling exceptionHandling = null)
            => new Ftp(hostIp, userName, password, exceptionHandling);

        public static void Validate(string hostIp, string userName, string password, ExceptionHandling exceptionHandling = null)
            => new Ftp(hostIp, userName, password, exceptionHandling).Validate();

        public void Validate() { this.Run(".", WebRequestMethods.Ftp.ListDirectory); }

        public static bool IsValid(string hostIp, string userName, string password, ExceptionHandling exceptionHandling = null)
            => new Ftp(hostIp, userName, password, exceptionHandling).IsValid();

        public bool IsValid() => CodeHelper.Catch(() =>
            {
                this.Validate();
                return true;
            },
        ...
        public async Task DownloadAsync(string remoteFile, string localFile) => await Async.Run(() => this.Download(remoteFile, localFile));
        ...
        public static void Upload(string remoteFile, string localFile, string hostIp, string userName, string password, ExceptionHandling exceptionHandling = null)
        {
            new Ftp(hostIp, userName, password, exceptionHandling).Upload(remoteFile, localFile);
        }

        public static void Download(string remoteFile, string localFile, string hostIp, string userName, string password, ExceptionHandling exceptionHandling = null)
        {
        ...
        private void Run(string path, string method, Action<StreamReader> action = null, Action disposer = null, Action<FtpWebRequest> initFtpWebRequest = null)
        {
            try
            {
        ...
        private TResult Run<TResult>(string path, string method, Func<StreamReader, TResult> action, Action disposer = null,
            TResult resultOnException = default(TResult))
        {
            try
            {
                var ftpRequest = (FtpWebRequest) WebRequest.Create(this._Host + "/" + path);
        ...
        {
            get { return this._ExceptionHandling ?? (this._ExceptionHandling = new ExceptionHandling {RaiseExceptions = true}); }
            private set { this._ExceptionHandling = value; }
        }

        public ILogger Logger { get; set; }
        public object DefaultSender => "Ftp";
    }
}