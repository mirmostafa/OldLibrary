...
namespace Mohammad.Web.Api.Tools
{
    public sealed class MutlipartTools
    {
        ...
        {
            var postedFile = CodeHelper.CatchFunc(() => HttpContext.Current?.Request.Files[index]).result;
        ...
        {
            return keys.Select(key => new
                                      {
                                          key,
                                          postedFile = HttpContext.Current?.Request.Files[key]
                                      }).Where(data => data.postedFile?.ContentLength > 0).Select(data => new WebPostedFile(data.postedFile, data.key));
        }
        ...
        {
            (bool isFound, int index) = FindMultipartKeyIndex(key);
            if (!isFound)
                return null;
            return HttpContext.Current?.Request.Form[index];
        }
